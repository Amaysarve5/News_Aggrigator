...
function TopHeadlines() {
  ...
  useEffect(() => {
    ...
    fetch(`https://news-aggrigator-server.onrender.com/top-headlines?language=en${categoryParam}&page=${page}&pageSize=${pageSize}`)
      .then((response) => {
        if (response.ok) {
      ...
      <div className='my-10 cards'>
        {!isLoading ? (
          data.length > 0 ? (
            ...
            <p className='text-center text-gray-500'>No articles found for this category.</p>
          )
        ) : (
          <Loader />
        )}
